// services/admin/webAuthnRegistrationService.ts
//
...
// the CSRF double-submit cookie. useApi() reads the rgl_csrf cookie and
// injects X-CSRF-Token automatically if the caller is cookie-authenticated.
...
export const createWebAuthnRegistrationService = (api: ApiInstance) => ({
  /**
   ...
   */
  getRegistrationOptions(): Promise<ApiEnvelope<WebAuthnRegistrationOptions>> {
   ...
   */
  verifyRegistration(challengeId: string, attestationResponse: unknown, friendlyName?: string): Promise<ApiEnvelope<WebAuthnVerifyRegistrationResult>> {
   ...
   */
  listCredentials(): Promise<ApiEnvelope<WebAuthnCredential[]>> {
    return api.get('/api/auth/webauthn/credentials');
  },

  /**
   * Delete a registered credential by its database row id.
   * The backend scopes the delete to the authenticated user.
   *
   * DELETE /api/auth/webauthn/credentials/:id
   * Returns: { success, message }
   */
  deleteCredential(id: number | string): Promise<ApiEnvelope<null>> {
