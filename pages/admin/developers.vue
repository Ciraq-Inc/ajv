...
const docGroups = computed(() => [
  ...
  { scope: 'knowledge_base', name: 'Knowledge Base', description: 'Read-only access to platform-wide, MedsGH-authored KB articles — runbooks/troubleshooting notes support agents reference while working tickets. Requires the support_tickets scope on a platform key, same gate as /tickets/manage/*. Always active-only; a platform-key agent never sees a draft/retired article. Full CRUD (create/update, including retiring an article) is JWT-only via the internal admin panel and not exposed to API keys.',
    endpoints: [
