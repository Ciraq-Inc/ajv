...
const docGroups = computed(() => [
  ...
  { scope: 'knowledge_base', name: 'Knowledge Base', description: 'Platform-wide, MedsGH-authored KB articles — runbooks/troubleshooting notes support agents reference while working tickets. All endpoints require the support_tickets scope on a platform key, same gate as /tickets/manage/*. GET is always active-only (a platform-key agent never sees a draft/retired article). This admin panel can still read and edit existing articles via /api/admin/knowledge-base, but creation happens here now, same key as everything else in this group.',
    endpoints: [
      ...
      { method: 'GET', path: '/api/v1/tickets/manage/kb/:id', description: 'PLATFORM KEYS ONLY. Fetch a single article. 404 if it does not exist or is not active (same message either way).',
        response: '{\n  "success": true,\n  "data": {\n    "id": 4,\n    "title": "Refund timelines for cancelled deliveries",\n    "body": "Refunds to the customer wallet post within 24 hours of...",\n    "category": "delivery",\n    "is_active": 1,\n    "created_by_admin_id": 2,\n    "created_at": "2026-07-30T09:00:00.000Z",\n    "updated_at": "2026-07-30T09:00:00.000Z"\n  }\n}' },
      { method: 'POST', path: '/api/v1/tickets/manage/kb', description: 'PLATFORM KEYS ONLY. Creates a new article; is_active defaults to true. category, if given, must be one of the ticket categories (order, payment, delivery, account, technical, billing, other). created_by_admin_id is always null here — there\'s no admin session behind a platform-key request to attribute it to.',
        body: '{\n  "title": "Refund timelines for cancelled deliveries",\n  "body": "Refunds to the customer wallet post within 24 hours of...",\n  "category": "delivery"\n}',
