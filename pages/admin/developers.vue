...
const docGroups = computed(() => [
  ...
  { scope: 'knowledge_base', name: 'Knowledge Base', description: 'Read-only access to platform-wide, MedsGH-authored KB articles — runbooks/troubleshooting notes support agents reference while working tickets. GET endpoints require the support_tickets scope on a platform key, same gate as /tickets/manage/*, and are always active-only (a platform-key agent never sees a draft/retired article). Authoring (create/update/retire) is JWT-only via this admin panel, not exposed to API keys — see the POST endpoint below, which uses your admin session token instead of x-api-key.',
    endpoints: [
      { method: 'GET', path: '/api/v1/tickets/manage/kb?category=delivery&search=refund', description: 'PLATFORM KEYS ONLY. Search/list active KB articles. Optional filters: category (matches the ticket category taxonomy), search (matches title/body).',
        response: '{\n  "success": true,\n  "count": 1,\n  "data": [\n    {\n      "id": 4,\n      "title": "Refund timelines for cancelled deliveries",\n      "body": "Refunds to the customer wallet post within 24 hours of...",\n      "category": "delivery",\n      "is_active": 1,\n      "created_by_admin_id": 2,\n      "created_at": "2026-07-30T09:00:00.000Z",\n      "updated_at": "2026-07-30T09:00:00.000Z"\n    }\n  ]\n}' },
      { method: 'GET', path: '/api/v1/tickets/manage/kb/:id', description: 'PLATFORM KEYS ONLY. Fetch a single article. 404 if it does not exist or is not active (same message either way).',
        response: '{\n  "success": true,\n  "data": {\n    "id": 4,\n    "title": "Refund timelines for cancelled deliveries",\n    "body": "Refunds to the customer wallet post within 24 hours of...",\n    "category": "delivery",\n    "is_active": 1,\n    "created_by_admin_id": 2,\n    "created_at": "2026-07-30T09:00:00.000Z",\n    "updated_at": "2026-07-30T09:00:00.000Z"\n  }\n}' },
      { method: 'POST', path: '/api/admin/knowledge-base', authHeaderOverride: 'Authorization: Bearer <admin JWT>', description: 'ADMIN JWT ONLY — not an x-api-key endpoint, so it will not work from the Test Console tab or a plain API key. Requires an admin session with role support_agent or higher (support_agent, auditor, manager, admin, super_admin). Creates a new article; is_active defaults to true. category, if given, must be one of the ticket categories (order, payment, delivery, account, technical, billing, other).',
        body: '{\n  "title": "Refund timelines for cancelled deliveries",\n  "body": "Refunds to the customer wallet post within 24 hours of...",\n  "category": "delivery"\n}',
        response: '{\n  "success": true,\n  "data": {\n    "id": 4,\n    "title": "Refund timelines for cancelled deliveries",\n    "body": "Refunds to the customer wallet post within 24 hours of...",\n    "category": "delivery",\n    "is_active": 1,\n    "created_by_admin_id": 2,\n    "created_at": "2026-07-30T09:00:00.000Z",\n    "updated_at": "2026-07-30T09:00:00.000Z"\n  }\n}' },
    ]},
...
const buildCurl = (ep: any) => {
  ...
  const authHeader = ep.noAuth ? '' : ep.authHeaderOverride ? ` \\\n  -H "${ep.authHeaderOverride}"` : ` \\\n  -H "x-api-key: <your-api-key>"`
